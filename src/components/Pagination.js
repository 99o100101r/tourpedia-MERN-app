...
const Pagination = ({ setCurrentPage, noOfPages, currentPage, dispatch }) => {
  const renderPaginaton = () => {
    if (currentPage === noOfPages && currentPage === 1) return null;
    if (currentPage === 1) {
      return (
  ...
  return <div className="mt-4">{renderPaginaton()}</div>;
};
